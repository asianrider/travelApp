...
class _EditProfileState extends State<EditProfile> {
  ...
  Future handleUpdateData(BuildContext context) async {
        ...
        if (formKey.currentState!.validate()) {
          ...
          imageFile == null
              ? await sb.updateUserProfile(nameCtrl.text, imageUrl!).then((value) {
                  context.openSnackBar('updated successfully'.tr());
                  setState(() => loading = false);
                })
              : await uploadPicture().then((value) => sb.updateUserProfile(nameCtrl.text, imageUrl!).then((_) {
                    context.openSnackBar('updated successfully'.tr());
                    setState(() => loading = false);
                  }));
        }
