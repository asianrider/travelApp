...
CustomRenderMatcher tagMatcher(String tag) => (context) => context.tree.element?.localName == tag;
  ...
  Widget build(BuildContext context) {
    return Html(
      ...
      style: {
        ...
        "figure": Style(margin: Margins.all(0), padding: EdgeInsets.zero),

        //Disable this line to disble full width image/video
        "p,h1,h2,h3,h4,h5,h6": Style(margin: Margins.all(20)),
      },
      customRenders: {
        tagMatcher("img"): CustomRender.widget(widget: (RenderContext context1, buildChildren) {
          final String _imageSource = context1.tree.element!.attributes['src'].toString();
          if (isimageEnabled == false) return Container();
          return InkWell(
            child: Container(
                padding: EdgeInsets.only(top: 10, bottom: 10), child: CachedNetworkImage(imageUrl: _imageSource)),
            onTap: () => nextScreen(context, FullScreenImage(imageUrl: _imageSource)),
          );
        }),
        tagMatcher("video"): CustomRender.widget(widget: (RenderContext context1, buildChildren) {
        ...
        tagMatcher("iframe"): CustomRender.widget(widget: (RenderContext context1, buildChildren) {
          final String _videoSource = context1.tree.element!.attributes['src'].toString();
          ...
          return Text("?? video source not supported");
        }),
      },
    );
  }
